...
final class ProductionMarketsStore: ObservableObject {
    ...
    init(client: StatisticsNorwayMarketClient = .init()) {
        self.client = client
    }
    ...
    func reload() async {
        state = .loading
        do {
            let series = try await client.fetchFreshSalmonSeries()
            state = .loaded([series])
        } catch {
            state = .unavailable(
...
struct StatisticsNorwayMarketClient {
    private static let endpoint = URL(
        string: "https://data.ssb.no/api/pxwebapi/v2/tables/03024/data?lang=en&valueCodes%5BVareGrupper2%5D=01&valueCodes%5BContentsCode%5D=*&valueCodes%5BTid%5D=top%2852%29&outputFormat=json-stat2"
    )!
    ...
    func fetchFreshSalmonSeries() async throws -> ProductionMarketSeries {
        ...
        for (timeIndex, periodCode) in timeCodes.enumerated() {
            let priceOffset = priceIndex * timeCount + timeIndex
            let weightOffset = weightIndex * timeCount + timeIndex
            guard
